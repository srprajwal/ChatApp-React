      ...
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        <div className="grid grid-cols-2 ">
          <div className="h-full w-32 md:w-48 md:h-48 relative flex items-center justify-center" onMouseEnter={() => setHovered(true)}
            ...
            {
              hovered && (<div className="absolute inset-0 flex justify-center items-center bg-black/50 ring-fuchsia-50 cursor-pointer rounded-full" onClick={image ? handleDeleteImage : handleFileInputClick}> {image ? <FaTrash className="text-white cursor-pointer text-3xl" /> : <FaPlus className="text-white cursor-pointer text-3xl" />}</div>
              )
            }
            <input type="file" ref={fileInputRef} className="hidden" onClick={handleImageChange} name="profile-image" accept=".png,
            .jpg, .jpeg, .webp, .svg" />
          </div>
